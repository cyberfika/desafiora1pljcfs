...
// ===== Miniscope estrito + wrapper Prenex robusto =====

// Usa freeVars(t) já definido acima.
// Checagem de ocorrência livre
function occursFree(v, node) {
  return freeVars(node).has(v);
}

// Miniscope estrito (não duplica em casos inseguros)
function miniscope(n){
  if(!n || typeof n !== 'object') return n;
  switch(n.k){
    case 'Forall': {
      const x = n.v;
      const A = miniscope(n.f);
      if (A.k === 'And'){
        const L = occursFree(x, A.l) ? Node.Forall(x, A.l) : A.l;
        const R = occursFree(x, A.r) ? Node.Forall(x, A.r) : A.r;
        return Node.And(miniscope(L), miniscope(R));
      }
      if (A.k === 'Or'){
        const inL = occursFree(x, A.l), inR = occursFree(x, A.r);
        if (inL && !inR) return Node.Or(miniscope(Node.Forall(x, A.l)), A.r);
        if (!inL && inR) return Node.Or(A.l, miniscope(Node.Forall(x, A.r)));
        if (!inL && !inR) return Node.Or(A.l, A.r);
        // inL && inR: não empurrar
      }
      return Node.Forall(x, A);
    }
    case 'Exists': {
      const x = n.v;
      const A = miniscope(n.f);
      if (A.k === 'Or'){
        const L = occursFree(x, A.l) ? Node.Exists(x, A.l) : A.l;
        const R = occursFree(x, A.r) ? Node.Exists(x, A.r) : A.r;
        return Node.Or(miniscope(L), miniscope(R));
      }
      if (A.k === 'And'){
        const inL = occursFree(x, A.l), inR = occursFree(x, A.r);
        if (inL && !inR) return Node.And(miniscope(Node.Exists(x, A.l)), A.r);
        if (!inL && inR) return Node.And(A.l, miniscope(Node.Exists(x, A.r)));
        if (!inL && !inR) return Node.And(A.l, A.r);
        // inL && inR: não empurrar
      }
      return Node.Exists(x, A);
    }
    case 'And': return Node.And(miniscope(n.l), miniscope(n.r));
    case 'Or':  return Node.Or(miniscope(n.l), miniscope(n.r));
    case 'Not': return Node.Not(miniscope(n.f));
    default:    return n;
  }
}

// Wrapper: aplica miniscope antes do seu coletor prenexStrict
function prenexRobust(f){
  const mini = miniscope(f);
  return prenexStrict(mini);
}
...
// Detecção correta de cláusulas de Horn
...
class Engine {
  ...
  cnfPrenex(ast){
    ...
    const pulled = prenexRobust(skol);
  ...
  dnfPrenex(ast){
    ...
    const pulled = prenexRobust(prenex);
...
function setExample(formula) {
  ...
  // remove quebras de linha e espaços extras no início/fim
